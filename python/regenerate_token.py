...
def regenerate_token(token,serial_number):
    ...
    if(results.ok):
        print ('--> Regenerating token for serial number: {} ...'.format(serial_number))
        jData = json.loads(results.content)
...
parser=argparse.ArgumentParser(description='FortiFlex: Regenerate token for specific serial number. API username and password need to be provided using environment variables.')
